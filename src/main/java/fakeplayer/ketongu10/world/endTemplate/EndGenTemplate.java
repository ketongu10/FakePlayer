package fakeplayer.ketongu10.world.endTemplate;
...
public class EndGenTemplate implements IChunkGenerator
{
		//Block that replaces End Stone
		protected static final IBlockState MAIN_BLOCK = Blocks.REDSTONE_ORE.getDefaultState();
		...
		public EndGenTemplate(World world, boolean mapFeaturesEnabled, long seed, BlockPos spawn)
	    {
	        this.world = world;
	        ...
	        spawnableCreatureList.add(new SpawnListEntry(EntityEvoker.class, 40, 38, 59));
	    ...
	    public static float getIslandHeightValue(int x, int z, int p_185960_3_, int p_185960_4_)
	    {
	        ...
	        for (int i = -12; i <= 12; ++i)
	        {
	            for (int j = -12; j <= 12; ++j)
	            {
	                ...
	                if (k * k + l * l > 4096L && EndGenTemplate.islandNoise.getValue((double)k, (double)l) < -0.8999999761581421D)
	                {
