package fakeplayer.ketongu10.entity;
...
public class EntityDiamondHook extends EntityFishHook
{
    ...
    private EntityDiamondHook.State currentState = EntityDiamondHook.State.FLYING;
    private int luck;
    private int lureSpeed;

    @SideOnly(Side.CLIENT)
    public EntityDiamondHook(World worldIn, EntityPlayer p_i47290_2_, double x, double y, double z)
    {
    ...
    public EntityDiamondHook(World worldIn, EntityPlayer fishingPlayer)
    {
	...
	public void onUpdate()
    {
        ...
        else if (this.world.isRemote || !this.shouldStopFishing())
        {
            ...
            if (this.currentState == EntityDiamondHook.State.FLYING)
            {
                if (this.caughtEntity != null)
                {
                    ...
                    this.motionY = 0.0D;
                    this.motionZ = 0.0D;
                    this.currentState = EntityDiamondHook.State.HOOKED_IN_ENTITY;
                    return;
                }
                ...
                if (f > 0.0F)
                {
                    ...
                    this.motionY *= 0.2D;
                    this.motionZ *= 0.3D;
                    this.currentState = EntityDiamondHook.State.BOBBING;
                    return;
                }
            ...
            {
                if (this.currentState == EntityDiamondHook.State.HOOKED_IN_ENTITY)
                {
                    if (this.caughtEntity != null)
                    {
                        if (this.caughtEntity.isDead)
                        {
                            ...
                            this.currentState = EntityDiamondHook.State.FLYING;
                        }
                        else
                        {
                ...
                if (this.currentState == EntityDiamondHook.State.BOBBING)
                {
                    this.motionX *= 0.9D;
                    this.motionZ *= 0.9D;
    ...
    private boolean shouldStopFishing()
    {
        ...
        boolean flag = itemstack.getItem() instanceof DiamondFishingRod;
        boolean flag1 = itemstack1.getItem() instanceof DiamondFishingRod;
