package fakeplayer.ketongu10.objects.blocks.silver_chest;
...
public class TileEntitySilverChest extends TileEntityLockableLoot implements ITickable 
{
	...
	{
		return this.hasCustomName() ? this.customName : "Silver Chest";
	}
	...
	public void update() 
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerSilverChest)
                {
                    if (((ContainerSilverChest)entityplayer.openContainer).getChestInventory() == this)
                    {
