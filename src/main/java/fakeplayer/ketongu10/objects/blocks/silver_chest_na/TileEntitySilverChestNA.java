package fakeplayer.ketongu10.objects.blocks.silver_chest_na;
...
public class TileEntitySilverChestNA extends TileEntityLockableLoot implements ITickable
{
	private NonNullList<ItemStack> chestContents = NonNullList.<ItemStack>withSize(30, ItemStack.EMPTY);
	...
	{
		return this.hasCustomName() ? this.customName : "container.silver_chest_na";
	}
	...
	{
		return new ContainerSilverChestNA(playerInventory, this, playerIn);
	}
	
	@Override
	public String getGuiID() 
	{
		return Reference.MOD_ID + ":silver_chest_na";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerSilverChestNA)
                {
                    if (((ContainerSilverChestNA)entityplayer.openContainer).getChestInventory() == this)
                    {
